...
| 計劃項目           | 進度     | 備註                           |
| ------------------ | -------- | ------------------------------ |
| 可靠度測試         | 進行中   | 目前張緊次數 155,000+ (2024/09/03) [測試影片](https://youtu.be/4xY9-XTofpA)|
| 適用網球拍         | 可靠度測試中 | 製作方法將在可靠度測試達到10萬次張緊後公佈                     |
| 相容 Pico 2       | 尚未開始 |                      |
...
## Q: 剛組裝好的機器無法進行 HX 張力校正，直接導致拉斷線。
A: 請確認是否安裝了 20KG 的 NJ5 傳感器 (YZC-133)。如果安裝的是 50KG 的版本，校正時張力會落在約 48LB 左右，這將導致無法正常校正。

## Q: 有時馬達會打滑
A: 如果在達到一定張力時出現打滑現象，請嘗試更換更大安培數的電源供應器，推薦使用 19V 4A 以上的規格。TB6600 支援寬電壓範圍，從 DC 9V 到 35V 都可以使用，因此理論上任何 19V 且 4A 以上的電源供應器都應該適用。此外，請確保 5V 轉換器能夠承受輸入電壓並穩定輸出 5V。如果問題仍然存在，請在討論區詳細描述情況，我可以幫助分析原因。
