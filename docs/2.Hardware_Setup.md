...
> [!WARNING]
> Be sure to watch **Hardware Procurement Recommendations**

> [!WARNING]
> Unless you have the ability to modify the program yourself, please purchase materials according to the specified models or specifications.
...
### Securing the Clamp Head and YZC-133 Sensor

Through subsequent testing, it was found that the YZC-133 sensor and clamp head do not require the copper standoffs mentioned in [EP5](https://youtu.be/PhSWcHrOhvc). Simply tightening the four screws is sufficient to withstand 120LB of tension without displacement.

### YZC-133 Tension Sensor

The NJ5 (YZC-133) sensor is used in this project primarily because it can be easily mounted on the WISE 2086 clamp head. If using a different clamp, other models can be chosen as long as they support at least 20KG of load. See the [SparkFun HX711](https://learn.sparkfun.com/tutorials/load-cell-amplifier-hx711-breakout-guide/all) guide for detailed instructions.

![img_yzc-133](img_yzc-133.jpg)

If the NJ5 YZC-133 sensor from Nanjing Measurement is unavailable, other YZC-133 sensors may have exposed four-wire signal leads. It is recommended to use a Ø4 spiral wrap to cover the wires, with a suggested wrapping length of 25 cm. After securing the sensor to the linear stage, use another Ø4 spiral wrap to bundle the clamp head wires with the sensor wires.

> [!WARNING]
> It is strongly discouraged to cut and rejoin the YZC-133 sensor wires, as such connections can lead to signal drift and startup warnings, preventing normal operation.

> [!WARNING]
> **It is highly recommended to purchase the NJ5 YZC-133 sensor**. General YZC-133 sensors are not tested for reliability and may have unknown issues.
...
If you are interested in the manufacturing process but are unsure if you can complete it successfully, it is recommended to follow the steps **gradually** to ensure that each step is completed smoothly.
...
> [!WARNING]
> `hx711.py`, `lcd_api.py`, and `pico_i2c_lcd.py` **must be placed inside the `src` folder**.
...
|----|---------------------------------------|----|------|
| 1  | Raspberry PI Pico H                   | 1  |      |
| 2  | SGX 1610 200mm Linear Stage with 56×57 1.2Nm Stepper Motor | 1  |      |
...
Gerber PCB [Download Fabrication Files](Gerber_PicoBETH_PCB_2024-10-31.zip)
...
| 13 | Raspberry Pi Pico H Microcontroller | 1   |
...
| 12 | XH2.54mm 6P (Same Direction)             | 1  | Five-Way Button for Button Board |
| 13 | XH2.54mm 4P (Same Direction)             | 2  | LED, 2004 LCD for Button Board |
| 14 | XH2.54mm 2P (Same Direction)             | 1  | Exit Button for Button Board |
...
| 21 | M3×6mm Round Head Screws                 | 10 |      |
| 22 | M3×6mm Self-Tapping Screws               | 4  |      |
| 23 | Cable Tie Mounts                         | 2  |      |
| 24 | M3×30mm Male-Female Brass Standoff       | 2  |      |
| 25 | M3×15mm Male-Female Brass Standoff       | 2  |      |
...
#### Calibration Steps:
1. Go to the settings page and ensure the automatic constant-pull function is enabled.
2. Set the HX parameter to **20.00**.
3. Return to the main menu and set the tension to **20.0 lbs** with **10% pre-stretch**.
4. Use an external tension gauge to measure the actual tension.
5. Enter the measured value into the settings page and save.

> [!IMPORTANT]
> If this calibration is not performed, the actual tension may not match the LCD-displayed tension.
