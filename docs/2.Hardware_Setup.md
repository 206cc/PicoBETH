...
### MicroPython 1.25 Requirement

> [!IMPORTANT]
> **Please use MicroPython (mpython) version 1.25.0.**
>
> The PicoBETH project was developed and tested using **mpython 1.25.0**.  
> Based on actual testing, **versions 1.26.0 and later** will cause the stepper motor to run significantly slower (about **30% slower**), which may affect overall performance and tension control.  
> In addition, **newer versions that have not been tested** may introduce other unknown issues.
>
> To avoid problems, **please use the provided 1.25.0 firmware file in this guide**.

> [!TIP]
> You can check the **MR value** in the **INFO screen**:
>
> - **MR around 1.10–1.25**: Normal (mpython 1.25.0)
> - **MR around 0.8x**: Likely using an incorrect version (1.26.0 or newer)

### Verify and Install MicroPython 1.25.0

If your Pico is not running **mpython 1.25.0**, please follow the steps below to update the firmware.

#### 1. Download the correct firmware

Download the appropriate `.uf2` firmware file based on your board:

- **Pico W / Pico WH**: [RPI_PICO_W-20250415-v1.25.0.uf2](RPI_PICO_W-20250415-v1.25.0.uf2)
- **Pico / Pico H**: [RPI_PICO-20250415-v1.25.0.uf2](RPI_PICO-20250415-v1.25.0.uf2)

> [!WARNING]
> **Do not mix firmware files. Pico W/WH and Pico/H use different firmware.**

#### 2. Enter boot mode

1. Press and hold the **BOOT** button on the Pico
2. Connect the Pico to your computer via USB
3. Release the **BOOT** button after plugging in

A new USB drive should appear on your computer (usually named **RPI-RP2**).

#### 3. Copy the firmware file

Copy the downloaded `.uf2` file directly into the Pico drive.

- No need to run or extract anything
- Just drag and drop or copy-paste the file

#### 4. Wait for automatic reboot

After copying, the Pico will automatically reboot and the USB drive will disappear.  
This means the firmware has been successfully installed.

#### 5. Verify the version in Thonny

1. Open **Thonny**
2. Select **MicroPython (Raspberry Pi Pico)** as the interpreter (bottom-right corner)
3. Connect to the Pico
4. Check the version information in the **Shell** window

Make sure the version is **1.25.0**.

If not, repeat the steps above and reflash the correct firmware.

> [!CAUTION]
> **Do not upgrade the MicroPython version unless tested.**
>
> This project is not guaranteed to work correctly on all MicroPython versions.  
> Even if the code runs, different versions may cause:
>
> - Incorrect stepper motor speed
> - Inconsistent tension behavior
> - Other unknown compatibility issues
>
> If you encounter any issues, **first verify that you are using mpython 1.25.0**.
...
> [!IMPORTANT]
> The PicoBETH MAIN PCB has been updated to **V2.0**.
>
> The assembly tutorial video was recorded using the **V1.8 PCB**, so the V1.8 Gerber files and instructions are still kept below for users who want to follow the video step by step.
>
> If you are building PicoBETH for the first time, it is recommended to use the newer **V2.0 PCB**. V2.0 adds reverse-polarity protection, updates the SparkFun HX711 VDD power configuration, and improves the installation and recommendations for the 5V step-down power module.
>
> [PicoBETH MAIN V2.0 PCB Update Notes and Downloads](https://github.com/206cc/PicoBETH/discussions/81)
...
> [!WARNING]
> You can either download the panelized version containing both the main board and button board, or download the two individual board files and have them manufactured separately.
>
> **Note:** Some PCB manufacturers may charge an additional fee for panelized PCB files.

> [!NOTE]
> After downloading the Gerber files, send them to a PCB manufacturer such as **JLCPCB, PCBWay, or another local PCB fabrication service**.
>
> Recommended manufacturing specifications: **2-layer PCB, 1.6 mm board thickness, plated through-holes**.
>
> Once the manufacturer confirms the specifications and provides a quote, you can proceed with the order.


### DC-DC 5V Power Supply Recommendation

PicoBETH uses a DC-DC step-down converter to reduce the main power supply voltage to a stable **5V** for the Pico, HX711, and related circuits.

The quality of generic **MP1584EN step-down modules** can vary significantly. Some low-cost modules may have unstable output voltage or excessive **ripple noise**, which can affect system stability and may cause issues such as **HX711 tension reading drift, unexpected button inputs, or other intermittent problems**.

For better reliability, the following 5V step-down modules are recommended:

1. **Pololu D24V22F5** — First choice. Recommended for its stable output and overall build quality.
2. **DM02-28050016DS** — Second choice. Uses the **MP1584EN** chip and has been tested to work reliably with PicoBETH.

Starting from **PCB V1.8**, the PCB includes a dedicated footprint for the **Pololu D24V22F5**. This support is also retained in **PCB V2.0**.

In **PCB V2.0**, the DC-DC module is mounted on the **front side of the PCB**, making it easier to measure the 5V output, replace the module, and perform maintenance after assembly.

> [!WARNING]
> If you use a generic **MP1584EN step-down module**, check that it provides a stable **5V output** before installing it on the PCB.
>
> If you are unsure about the quality of the module, it is recommended to use the **Pololu D24V22F5** or **DM02-28050016DS** instead.
...
| 9  | XH2.54mm 4P Connector 38cm               | 1  | TB6600 Signal Wire |
...
![img_3d_control_box](img_3d_control_box.jpg)

A 3D printed control box designed for PicoBETH HW2.

This enclosure integrates the LCD, button panel, and main board mounting positions into a more compact and organized layout, reducing additional brackets and mechanical fabrication work.

Supports both 2004A and 2004C LCD versions, and can be used directly with the original HW2 structure or adapted for custom DIY builds.

More information and download:
- [HW2 Control Box](https://github.com/206cc/PicoBETH/discussions/68)